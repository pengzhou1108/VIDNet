        ...
        if self.split == 'val':
            ...
            for batch_idx, (inputs, targets,seq_name,starting_frame) in enumerate(self.loader):
                ...
                for ii in range(max_ii):
                    ...
                    outs_edge, outs, hidden_temporal_list = test_edge(args, self.encoder, self.decoder, x, prev_hidden_temporal_list)
                    ...
                    for t in range(1):
                        ...
                        edge_pred = (torch.squeeze(outs_edge[0, t, :])).cpu().numpy()
                        edge_pred = np.reshape(edge_pred, (height, width))
                        #pdb.set_trace()
                    ...
                    if args.overlay_masks:
                        ...
                        figname = base_dir + 'frame_%02d.jpg' %(starting_frame[0]+ii)
                        ...
                        for t in range(num_instances):
                            ...
                            mask_pred = (torch.squeeze(outs_edge[0,t,:])).cpu().numpy()
                            mask_pred = np.reshape(mask_pred, (height, width))
                            ax = plt.gca()
                            tmp_img = np.ones((mask_pred.shape[0], mask_pred.shape[1], 3))
                            color_mask = np.array(colors[t])/255.0
                        ...
                        figname = base_dir + 'frame_edge_%02d.jpg' %(starting_frame[0]+ii)
                        plt.savefig(figname,bbox_inches='tight')
                        plt.close()
