...
class FeatureExtractor(nn.Module):
    ...
    def __init__(self,args):
        ...
        if args.base_model == 'resnet34':
            ...
            self.base = UNet(n_channels=6, n_classes=1)
    ...
    def forward(self,x,semseg=False, raw = False,x_ela=None):
        ...
        if x_ela is not None:
            ...
            if True:
                ...
                #return torch.cat([x5,x51],1), torch.cat([x4,x41],1),torch.cat([x3,x31],1), torch.cat([x2,x21],1), torch.cat([x1,x11],1)
...
class RSIS(nn.Module):
    ...
    def __init__(self, args):
        ...
        if True:
            ...
            #self.aspp_f.append(_ASPPModule(self.hidden_size, self.hidden_size, 1, padding=0, dilation=dilations[0], BatchNorm=nn.BatchNorm2d))
            #self.aspp_f.append(_ASPPModule(self.hidden_size, self.hidden_size, 3, padding=dilations[1], dilation=dilations[1], BatchNorm=nn.BatchNorm2d))
        ...
        #self.conv_out_edge = nn.ModuleList()
        ...
        self.conv_out_edge = nn.Conv2d(512+256+128, 1,self.kernel_size, padding = padding)
    ...
    def forward(self, skip_feats, prev_state_spatial, prev_hidden_temporal, T=0, x=None):     
        ...
        #clstm_in = self.aspp_f[T](clstm_in)
        #pdb.set_trace()
        ...
        for i in range(len(skip_feats)+1):
            ...
            if i < len(skip_feats):
                ...
                # skip connection
                if self.skip_mode == 'concat':
                    #clstm_in = torch.cat([hidden,skip_vec, x],1)
        ...
        edge_feature = torch.cat([upsample_match(f) for f in skip_feats[:-1]],1)
        #edge_feature = torch.cat([upsample_match(f) for f in skip_feats[:-1]],1)
        #edge_feature = torch.cat([upsample_match(f[0]) for f in hidden_list[1:-1]],1)
        #edge_feature1 = torch.cat([upsample_match(f[0]) for f in hidden_list[1:-1]],1)
        edge_mask = self.conv_out_edge(edge_feature)
        #edge_mask1 = self.conv_out_edge(edge_feature1)

        #seg_mask = self.conv_out_seg(torch.cat([clstm_in,torch.nn.Sigmoid()(edge_mask)],1))
        if T>0:
        #pdb.set_trace()
            clstm_in = recursive_filter_edge(clstm_in,torch.nn.Sigmoid()(edge_mask),T)

        seg_mask = self.conv_out_seg(clstm_in)
        out_mask = seg_mask
        #out_mask = self.conv_out(torch.cat([edge_mask,edge_mask1,seg_mask],1))
        #out_mask = self.conv_out(torch.cat([edge_mask,seg_mask],1))
        
        # classification branch

        return edge_mask, out_mask, hidden_list
