...
def test_edge(args, encoder, decoder, x, prev_hidden_temporal_list):
    ...
    for t in range(0, T):
        ...
        out_edge, out_mask, hidden = decoder(feats, hidden_spatial, hidden_temporal,T=t,x=x[:,3:,:,:])
        hidden_tmp = []
        ...
        out_edge = upsample_match(out_edge)
    ...
    if T<2:
        ...
        out_masks = decoder.conv_out(torch.cat(out_masks,1)).view(x.size(0),1, -1)
    out_edges = torch.cat(out_edges,1).view(out_edges[0].size(0),len(out_edges), -1)
    #out_masks = decoder.conv_out(torch.cat(out_masks,1)).view(x.size(0),1, -1)
    out_masks = torch.sigmoid(out_masks)
    out_edges = torch.sigmoid(out_edges)
    outs = out_masks.data
    outs_edge = out_edges.data
    
    del feats, x
    return outs_edge, outs, hidden_temporal_list
def test_ela(args, encoder, decoder, x, prev_hidden_temporal_list,x_ela=None):
    ...
    for t in range(0, T):
        ...
        #hidden state from the previous object instance from the same time instant, and hidden_temporal which refers to the hidden state from the same
        #object instance from the previous time instant.
        _, out_mask, hidden = decoder(feats, hidden_spatial, hidden_temporal,T=t)
        hidden_tmp = []
        for ss in range(len(hidden)):
            hidden_tmp.append(hidden[ss][0].data)
        hidden_spatial = hidden
...
def test(args, encoder, decoder, x, prev_hidden_temporal_list,x_ela=None):
