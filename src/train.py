...
from utils.objectives import softIoULoss, MaskedBCELoss, DiceLoss
import time
import math
import os
import warnings
...
def runIter(args, encoder, decoder, x, y_mask, sw_mask,
    ...
    loss_mask_iou = mask_siou(y_mask_perm.view(-1,y_mask_perm.size()[-1]),out_masks.view(-1,out_masks.size()[-1]),None)
...
def trainIters(args):
    ...
    if args.optim=='sgd':
        de_scheduler = torch.optim.lr_scheduler.StepLR(dec_opt, step_size=15, gamma=0.1)
    if args.optim_cnn=='sgd':
        scheduler = torch.optim.lr_scheduler.StepLR(enc_opt, step_size=15, gamma=0.1)
    if args.resume:
    ...
    for e in range(args.max_epoch):
        ...
        for split in ['train', 'val']:
            if args.dataset == 'davis2016' or args.dataset == 'youtube':
                for batch_idx, (inputs, targets,seq_name,starting_frame) in enumerate(loaders[split]):
                    ...
                    for ii in range(max_ii):
                        ...
                        if tensorboard_step % 200 == 0:
                            ...
                            #x_m = vutils.make_grid(true_masks.unsqueeze(1), normalize=True, scale_each=True)                        
