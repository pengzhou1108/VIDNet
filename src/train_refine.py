...
from modules.model import RSIS, FeatureExtractor,WarpingLayer
...
def runIter(args, encoder, decoder, x, x_org, y_mask, sw_mask,
            crits, optims, mode='train', permutation = None, loss = None, prev_hidden_temporal_list=None, last_frame=False,next_mask=None,flow=None,warp_layer=None, retain_graph=False):
    """
    ...
    if next_mask is not None:
        loss_mask_iou += 0.5*torch.mean(mask_siou(next_mask.view(next_mask.size()[0],-1).data,out_masks.view(-1,out_masks.size()[-1])))
                ...
                for batch_idx, (inputs, inputs1, inputs_org, targets,seq_name,starting_frame, imgs_flow) in enumerate(loaders[split]):
                    ...
                    for ii in range(max_ii):
                        ...
                        loss, losses, outs, permutation, hidden_temporal_list, outs_warp = runIter(args, encoder, decoder, x, x_org, y_mask, sw_mask,
                                                                                            ...
                                                                                            loss, prev_hidden_temporal_list, last_frame=last_frame, next_mask= next_mask, flow=flow, warp_layer=warp_layer,retain_graph=False)
                        if False:                    
                            #pdb.set_trace()
                            ...
                            x_refine = boundary*x_org \
                                        + x*(1-boundary)
                            y_mask_re = y_mask - y_mask*boundary.view(outs.shape)
                        ...
                        if tensorboard_step % 200 == 0:
                            ...
                            if False:
                                x_im_re = vutils.make_grid([inv_normalize(k) for k in x_refine], normalize=True, scale_each=True)
                                x_re = vutils.make_grid(outs_refine.view(outs_refine.shape[0],outs_refine.shape[1],x.shape[-2],x.shape[-1]), normalize=True, scale_each=True)
                                ...
                                writer.add_image('pred_refine', x_re, tensorboard_step)
