...
from modules.model_edge import RSIS, FeatureExtractor
...
def runIter(args, encoder, decoder, x,x_ela, x1,x1_ela, y_mask,y_edge, sw_mask,
    ...
    for t in range(0, T):
        ...
        if False:
            ...
            if False:
                ...
                edge1_mask = upsample_match(edge1_mask)
                edge1_mask = edge1_mask.view(edge1_mask.size(0), -1)
                edge1_mask1 = upsample_match(edge1_mask1)
                edge1_mask1 = edge1_mask1.view(edge1_mask1.size(0), -1)
                edge1_masks.append(edge1_mask)
                edge1_masks1.append(edge1_mask1)
    ...
    #edge1_masks = torch.cat(edge1_masks,1).view(edge1_mask.size(0),len(edge1_masks), -1)
    #edge1_masks1 = torch.cat(edge1_masks1,1).view(edge1_mask1.size(0),len(edge1_masks1), -1)
    ...
    if True:
        ...
        #loss_edge1_iou = edge_bce(y_edge_perm.view(-1,y_edge_perm.size()[-1]),edge1_masks.view(-1,edge1_masks.size()[-1]))
        #loss_edge1_iou = torch.mean(loss_edge1_iou)

        #loss_edge1_iou1 = edge_bce(y_edge_perm.view(-1,y_edge_perm.size()[-1]),edge1_masks1.view(-1,edge1_masks1.size()[-1]))
        #loss_edge1_iou1 = torch.mean(loss_edge1_iou1)     
    # total loss is the weighted sum of all terms
    if loss is None:
        loss = args.iou_weight * loss_mask_iou + args.iou_weight * loss_mask_iou1 + loss_edge_iou +loss_edge_iou1 #+ loss_edge1_iou +loss_edge1_iou1
